...
def quick_prefetch(urls, max_n: int = 4, deadline: float = float("inf")):
    """
...
def main():
    ...
    try:
        if can_spend("vertex", VERTEX_Q_PER_RUN):
            ...
            # ★ prefetch も軽く：最大4件だけ即保存を試みる
            quick_prefetch(extra, max_n=4, deadline=deadline)
        else:
