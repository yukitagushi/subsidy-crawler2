create table if not exists public.pages(
  ...
  title text not null,
  summary text,
  rate text, cap text, target text, cost_items text,
  deadline text, fiscal_year text, call_no text, scheme_type text,
  period_from text, period_to text, content_hash text,
...
create table if not exists public.api_quota(
  month text not null, api text not null, used integer not null default 0, limit integer not null,
